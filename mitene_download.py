  ...
  """,
    ...
    async with aiohttp.ClientSession(
        timeout=aiohttp.ClientTimeout(
            total=datetime.timedelta(minutes=30).total_seconds()
        )
    ) as session:
        page = 1
        while True:
            ...
            if page == 1 and "Please enter your password" in response_text:
                ...
                if r.url.path.endswith("/login"):
                    print(
                        "Could not authenticate, maybe password is incorrect",
                        file=sys.stderr,
                    )
                    sys.exit(1)
                continue
            ...
            for media in data["mediaFiles"]:
                ...
                if media["comments"]:
                    ...
                    comment_file = pathlib.Path(
                        os.path.splitext(destination_filename)[0] + ".md"
                    )
                    if not (
                        comment_file.exists()
